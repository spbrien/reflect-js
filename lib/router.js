    ...
    .get((req, res, next) => {
      ...
      if (includes) {
        ...
        ormIncludeParameter = R.filter(
          (item) => item,
          R.map((include) => {
            const mapping = createRelationshipMapping(req.params.resource, include, relationships)
            if (mapping) {
              const directMap = getDirectRelationshipMap(mapping)
              if (directMap) {
                // Create our simple relationships
                models[req.params.resource].hasMany(
                  models[include.resource],
                  {
                    foreignKey: directMap.target_column,
                    sourceKey: directMap.source_column,
                  }
                )

                const { filters, params } = createSimpleRelationship(
                  models,
                  include,
                  req.params.resource,
                  directMap
                ...
                // Add generated relationship filters
                ormIncludeFilters.push(filters)

                // return our query params for this relationship
                return params
              } else {
                const complexMap = getComplexRelationshipMap(mapping, include)
                if (complexMap) {
                  // Create our complex relationships
                  models[req.params.resource].belongsToMany(models[include.resource], {
                    through: complexMap.through,
                    foreignkey: complexMap.intermediate_source_column,
                    otherkey: complexMap.intermediate_target_column,
                  })

                  const { filters, params } = createComplexRelationship(
                    models,
                    include,
                    req.params.resource,
                    complexMap
      ...
      if (models[req.params.resource]) {
        query.count(req.params.resource, countParams).then((total) => {
          query.find(req.params.resource, params).then((items) => {
            if (items) {
              // Apply filters to returned database data
              const results = R.map((item) => {
                if (ormIncludeFilters.length > 0) {
                  return R.apply(R.compose, ormIncludeFilters)(item)
                }
                return item
              })(items)

              // Remove associations from Sequelize
              models[req.params.resource].associations = {}

              // Set the apiData for return in response
              req.apiData = {
                data: results,
          ...
          query.findOne(req.params.resource, originalParams).then((item) => {
            ...
            query.count(req.params.relationship, countParams).then((total) => {
              query.find(req.params.relationship, params).then((items) => {
                if (items) {
                  models[req.params.resource].associations = {}
                  models[req.params.relationship].associations = {}
                  req.apiData = {
                    data: items,
                    per_page: items.length,
                    page: req.query.page ? parseInt(req.query.page) : 1,
