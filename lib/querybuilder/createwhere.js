...
function createWhereQuery() {
  return R.curry((operator, column, comparison, value) => {
    if (whereOperators[comparison]) {
      //TODO: turns out we don't need this operator logic for and...
      const op = operator === 'and' ? null : 'or'
      const fn = getWhereFunctionName(op, whereOperators[comparison].funcName)
...
function whereFactory(schema, table, params) {
  if (params.where) {
    ...
    if (initializedWhereFunctions) {
      return (q) => {
        return composeList(R.flatten(initializedWhereFunctions))(q)
      }
    }
    return (q) => q
  }
