...
function createJoinFilter(models, related, resource) {
  ...
  const primaryColumns = R.map((column) => column.name, primaryModel.columns)
  const primaryObj = createEmptyResourceFromColumns(resource, primaryColumns)


  return R.map((result) => {
    console.log(selectMatchingColumns(primaryColumns, result))
    const primaryResult = R.merge(
      primaryObj[resource],
      selectMatchingColumns(primaryColumns, result)
    )
  ...
  return {
    find: (table, params) => {
      return connection.transaction((trx) => {
        ...
        if (include) {
          ...
          const joinFunctions = R.map((item) => {
            ...
            transformations.push(
              createJoinFilter(models, map.relatedTo, table)
            )
        ...
        .then((rows) => {
          // Map over row
          const results = R.apply(R.compose, transformations)(rows)
