...
function errorStatus(res, status) {
  return res
  ...
  router.route('/:resource')
    ...
    .get((req, res, next) => {
      ...
      if (includes) {
        ...
        if (mapping) {
          ormIncludeParameter = R.map((include) => {
            // TODO: finish creating includes
            // Include will look like: {
            //   resource: RelatedResource,
            //   direct: true | false,
            //   through: ThroughTable | null,
            //   where: {}
            // }
            const direct = R.filter((item) => item.direct, mapping)
            if (direct && direct.length > 0) {
              // TODO: Write direct relationship functionality
              // If we have a direct relationship, create a model for it
              // return {
              //  model: ourNewRelationshipModel,
              //  where: our where params
              // }
            } else {
              // TODO: Write conplex relationship functionality
            }
          }, includes)
        }
      }
