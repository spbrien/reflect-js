...
module.exports = function (models, relationships, query, options) {
  ...
  router.route('/:resource')
    ...
    .get((req, res, next) => {
      ...
      const attributes = req.query.attributes ? JSON.parse(req.query.attributes) : null
                ...
                if (models[include.resource]) {
                  ...
                  ormIncludeFilters.push(
                    applyFilters([
                      ...
                      getAttributes(include.attributes),
                    ])(include.resource)
                  )
                  return {
        ...
        query.count(req.params.resource, countParams).then((total) => {
          query.find(req.params.resource, params).then((items) => {
            if (items) {
              ...
              req.apiData = {
                data: attributes ? R.map(R.pick(attributes), results) : results,
