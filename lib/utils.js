    ...
    return (db) => {
      return db.leftOuterJoin(
        ...
        function() {
          ...
          // TODO: This is the query I need to build
          .onIn(
            `${map.relatedTo}.ProductTrialID`,
            function() {
              this.select(`${map.relatedTo}.ProductTrialID`).from(`${map.relatedTo}`).where('Trials.IsTopThree', false)
            }
          )
        }
      )
